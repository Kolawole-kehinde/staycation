export const data = [
  {
    id: "A1B2C3",
    name: "Tabby Town",
    location: "Gunung Batu, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807829/Tabby_Town_wiobfz.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807829/Tabby_Town_wiobfz.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807829/Tabby_Town_wiobfz.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807829/Tabby_Town_wiobfz.png",
    ],
    description: "Tabby Town offers a serene escape in the heart of nature, perfect for travelers looking to unwind. Nestled in Gunung Batu, Indonesia, this charming retreat is surrounded by lush greenery and breathtaking landscapes. <br><br> Experience a seamless blend of modern comforts and rustic charm, with well-furnished living spaces, cozy bedrooms, and high-speed internet to keep you connected. Whether you're relaxing on the balcony with a cup of coffee or exploring nearby trails, Tabby Town ensures an unforgettable stay. <br><br> Designed to cater to your comfort, this retreat is the ideal getaway for families, couples, and solo adventurers alike. Book now and indulge in tranquility!",
    pricePerNight: 280,
    duration: 2,
    features: {
      bedrooms: 3,
      bathrooms: 3,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "10 mbps",
      unitsReady: 2,
      refrigerators: 2,
      television: 4
    }
  },
  {
    id: "D4E5F6",
    name: "Anggana",
    location: "Bogor, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807834/Anggana_nehttt.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807834/Anggana_nehttt.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807830/Seattle_Rain_al4njs.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807832/Wooden_Pit_ezih1l.png"
    ],
    description: "Anggana is a modern villa situated in the heart of Bogor, Indonesia, offering stunning city views and a luxurious stay. <br><br> With contemporary interiors, spacious living areas, and high-speed WiFi, this retreat is designed for comfort and convenience. Relax in elegantly decorated bedrooms, enjoy a home-cooked meal in the dining area, or watch your favorite shows on the high-definition television. <br><br> Whether you're visiting for business or leisure, Anggana provides the perfect balance of tranquility and accessibility. Book your stay today and experience luxury like never before!",
    pricePerNight: 320,
    duration: 2,
    features: {
      bedrooms: 4,
      bathrooms: 3,
      livingRoom: 2,
      diningRoom: 1,
      internetSpeed: "15 mbps",
      unitsReady: 3,
      refrigerators: 1,
      television: 5
    }
  },
  {
    id: "G7H8I9",
    name: "Seattle Rain",
    location: "Jakarta, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807830/Seattle_Rain_al4njs.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807830/Seattle_Rain_al4njs.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807832/Wooden_Pit_ezih1l.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1741447715/green_park_qf3lvh.png"
    ],
    description: "Experience the pinnacle of luxury at Seattle Rain, a sophisticated penthouse in the heart of Jakarta, Indonesia. <br><br> With floor-to-ceiling windows, breathtaking cityscape views, and high-end amenities, this residence is designed for elite travelers. Enjoy spacious bedrooms, modern furnishings, and seamless connectivity with high-speed internet. <br><br> Whether you're lounging in the living room, dining in style, or working remotely, Seattle Rain delivers comfort and convenience at its finest. Secure your stay now and elevate your travel experience!",
    pricePerNight: 350,
    duration: 2,
    features: {
      bedrooms: 5,
      bathrooms: 4,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "25 mbps",
      unitsReady: 3,
      refrigerators: 2,
      television: 6
    }
  },
  {
    id: "J1K2L3",
    name: "Wooden Pit",
    location: "Wonosobo, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807832/Wooden_Pit_ezih1l.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807832/Wooden_Pit_ezih1l.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1741447715/green_park_qf3lvh.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Podo_Wae_o7yo21.png"
    ],
    description: "Escape to the Wooden Pit, a beautiful wooden cabin in the mountains of Wonosobo, Indonesia, perfect for nature lovers and adventure seekers. <br><br> This cozy retreat features a rustic design, offering a warm and inviting atmosphere. With a fully equipped kitchen, comfortable living spaces, and breathtaking mountain views, this is the ultimate getaway for relaxation. <br><br> Whether you're exploring nearby trails, enjoying a barbecue night, or stargazing from the balcony, Wooden Pit offers a refreshing retreat from city life. Book now for an unforgettable experience!",
    pricePerNight: 300,
    duration: 2,
    features: {
      bedrooms: 3,
      bathrooms: 2,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "12 mbps",
      unitsReady: 1,
      refrigerators: 1,
      television: 3
    }
  },
  {
    id: "M4N5O6",
    name: "Green Park",
    location: "Bandung, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1741447715/green_park_qf3lvh.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1741447715/green_park_qf3lvh.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Podo_Wae_o7yo21.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807834/Anggana_nehttt.png"
    ],
    description: "Green Park is a tranquil retreat nestled in the lush landscapes of Bandung, Indonesia. <br><br> Surrounded by nature, this retreat offers a peaceful escape with modern amenities. Enjoy spacious living areas, cozy bedrooms, and a fully equipped kitchen. <br><br> Whether you're relaxing on the patio, exploring nearby parks, or enjoying a family meal, Green Park provides the perfect setting for a rejuvenating getaway. Book your stay today!",
    pricePerNight: 290,
    duration: 2,
    features: {
      bedrooms: 4,
      bathrooms: 3,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "20 mbps",
      unitsReady: 2,
      refrigerators: 2,
      television: 4
    }
  },
  {
    id: "P7Q8R9",
    name: "Podo Wae",
    location: "Malang, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Podo_Wae_o7yo21.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Podo_Wae_o7yo21.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807834/Anggana_nehttt.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737807830/Seattle_Rain_al4njs.png"
    ],
    description: "Podo Wae is a charming retreat located in the scenic city of Malang, Indonesia. <br><br> This retreat offers a perfect blend of modern amenities and natural beauty, with spacious living areas, comfortable bedrooms, and stunning views of the surrounding landscape. <br><br> Whether you're exploring the city, relaxing in the garden, or enjoying a quiet evening indoors, Podo Wae ensures a memorable stay. Book now and experience the beauty of Malang!",
    pricePerNight: 310,
    duration: 2,
    features: {
      bedrooms: 3,
      bathrooms: 2,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "15 mbps",
      unitsReady: 2,
      refrigerators: 1,
      television: 3
    }
  },
  {
    id: "S1T2U3",
    name: "Silver Rain",
    location: "Bandung, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837616/Silver_Rain_ivdp8o.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837616/Silver_Rain_ivdp8o.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837616/Silver_Rain_ivdp8o.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837616/Silver_Rain_ivdp8o.png"
    ],
    description: "Silver Rain is a luxurious retreat located in the picturesque town of Lembang, Bandung, Indonesia. <br><br> With stunning mountain views, modern amenities, and spacious living areas, this retreat offers the perfect escape for relaxation and rejuvenation. Enjoy cozy bedrooms, a fully equipped kitchen, and a private balcony with breathtaking views. <br><br> Whether you're watching the sunrise, exploring nearby trails, or enjoying a quiet evening indoors, Silver Rain ensures a memorable stay. Book now and experience the magic of Lembang!",
    pricePerNight: 330,
    duration: 2,
    features: {
      bedrooms: 4,
      bathrooms: 3,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "18 mbps",
      unitsReady: 2,
      refrigerators: 2,
      television: 4
    }
  },
  {
    id: "V4W5X6",
    name: "Cashville Bliss",
    location: "Kemang, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Cashville_g00nxv.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Cashville_g00nxv.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Cashville_g00nxv.png",
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737837613/Cashville_g00nxv.png",
     
    ],
    description: "Cashville Bliss is a luxurious retreat located in the vibrant neighborhood of Kemang, Indonesia. <br><br> With stunning city views, modern amenities, and spacious living areas, this retreat offers the perfect escape for relaxation and rejuvenation. Enjoy cozy bedrooms, a fully equipped kitchen, and a private pool. <br><br> Whether you're exploring the city, relaxing by the pool, or enjoying a quiet evening indoors, Cashville Bliss ensures a memorable stay. Book now and experience the magic of Kemang!",
    pricePerNight: 400,
    duration: 2,
    features: {
      bedrooms: 5,
      bathrooms: 4,
      livingRoom: 2,
      diningRoom: 1,
      internetSpeed: "25 mbps",
      unitsReady: 3,
      refrigerators: 2,
      television: 5
    }
  },
  {
    id: "Y7Z8A9",
    name: "PS Wood",
    location: "Depok, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/PS_Wood_ol5kdg.png",
    images: [
     "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/PS_Wood_ol5kdg.png",
     "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/PS_Wood_ol5kdg.png",
     "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/PS_Wood_ol5kdg.png",
    ],
    description: "PS Wood is a stunning retreat located in the serene city of Depok, Indonesia. <br><br> With breathtaking views, modern amenities, and spacious living areas, this retreat offers the perfect escape for relaxation and rejuvenation. Enjoy cozy bedrooms, a fully equipped kitchen, and a private garden. <br><br> Whether you're exploring the city, relaxing in the garden, or enjoying a quiet evening indoors, PS Wood ensures a memorable stay. Book now and experience the beauty of Depok!",
    pricePerNight: 380,
    duration: 2,
    features: {
      bedrooms: 4,
      bathrooms: 3,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "20 mbps",
      unitsReady: 2,
      refrigerators: 2,
      television: 4
    }
  },
  {
    id: "B1C2D3",
    name: "One Five",
    location: "Jakarta, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/One_Five_ahski5.png",
    images: [
     "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/One_Five_ahski5.png",
     "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/One_Five_ahski5.png",
     "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841096/One_Five_ahski5.png",
    ],
    description: "One Five is a luxurious retreat located in the bustling city of Jakarta, Indonesia. <br><br> With stunning city views, modern amenities, and spacious living areas, this retreat offers the perfect escape for relaxation and rejuvenation. Enjoy cozy bedrooms, a fully equipped kitchen, and a private balcony. <br><br> Whether you're exploring the city, relaxing on the balcony, or enjoying a quiet evening indoors, One Five ensures a memorable stay. Book now and experience the magic of Jakarta!",
    pricePerNight: 270,
    duration: 2,
    features: {
      bedrooms: 3,
      bathrooms: 2,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "12 mbps",
      unitsReady: 1,
      refrigerators: 1,
      television: 3
    }
  },
  {
    id: "E4F5G6",
    name: "Minimal",
    location: "Bogor, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841094/Minimal_appued.png",
    images: [
             "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841094/Minimal_appued.png",
             "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841094/Minimal_appued.png",
             "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841094/Minimal_appued.png",
    ],
    description: "Minimal is a serene retreat located in the scenic city of Bogor, Indonesia. <br><br> With stunning river views, modern amenities, and spacious living areas, this retreat offers the perfect escape for relaxation and rejuvenation. Enjoy cozy bedrooms, a fully equipped kitchen, and a private garden. <br><br> Whether you're exploring the river, relaxing in the garden, or enjoying a quiet evening indoors, Minimal ensures a memorable stay. Book now and experience the beauty of Bogor!",
    pricePerNight: 290,
    duration: 2,
    features: {
      bedrooms: 3,
      bathrooms: 2,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "15 mbps",
      unitsReady: 2,
      refrigerators: 1,
      television: 3
    }
  },
  {
    id: "H7I8J9",
    name: "Stays Home",
    location: "Wonosobo, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841095/Stays_Home_lrdx2s.png",
    images: [
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841095/Stays_Home_lrdx2s.png", 
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841095/Stays_Home_lrdx2s.png", 
      "https://res.cloudinary.com/daarrcw3q/image/upload/v1737841095/Stays_Home_lrdx2s.png", 
    ],
    description: "Minimal is a serene retreat located in the scenic city of Bogor, Indonesia. <br><br> With stunning river views, modern amenities, and spacious living areas, this retreat offers the perfect escape for relaxation and rejuvenation. Enjoy cozy bedrooms, a fully equipped kitchen, and a private garden. <br><br> Whether you're exploring the river, relaxing in the garden, or enjoying a quiet evening indoors, Minimal ensures a memorable stay. Book now and experience the beauty of Bogor!",
    pricePerNight: 290,
    duration: 2,
    features: {
      bedrooms: 3,
      bathrooms: 2,
      livingRoom: 1,
      diningRoom: 1,
      internetSpeed: "15 mbps",
      unitsReady: 2,
      refrigerators: 1,
      television: 3
    }
  },
  {
    id: "K1L2M3",
    name: "Green Park",
    location: "Bandung, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1741692147/tobby_xiewac.png",
  },
  {
    id: "N4O5P6",
    name: "Green Park",
    location: "Bandung, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1741692146/dog_xogxfv.png",
  },
  {
    id: "Q7R8S9",
    name: "Green Park",
    location: "Bandung, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1741692146/labour_mtoav2.png",
  },
  {
    id: "T1U2V3",
    name: "Green Park",
    location: "Bandung, Indonesia",
    mainImage: "https://res.cloudinary.com/daarrcw3q/image/upload/v1741692146/snorkelling_t7kjqp.png",
  },
];